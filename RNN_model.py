...
PATH_DATA = '../data'
os.makedirs('./results/RNN_model', exist_ok=True)
...
for epoch in range(1, EPOCHS+1):
    ...
    if best_test_loss > loss.item():
        ...
        torch.save(model.cpu().state_dict(), f'./results/RNN_model/model-{epoch}.pt')
        model = model.to(DEVICE)


    plt.figure(figsize=(12,5))
    plt.title('RNN modelis')
    plts = []
    c = 0
    ...
    plt.savefig(f'./results/RNN_model/epoch-{epoch}.png')
    plt.show()